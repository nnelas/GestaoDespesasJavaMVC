...
/**
 * Created by nunonelas on 14/01/17.
 */

@Controller
@Transactional
public class MapaAgregadoController {

    @PersistenceContext
    private EntityManager em;

    @RequestMapping(value = "/mapaAgregado", method = RequestMethod.GET)
    public String getMap(ModelMap model,
        ...
        if (agregados.isEmpty()){
            model.addAttribute("message", "Ainda não adicionou nenhum elemento ao seu agregado familiar.");

            return "agregadoResult";
        } else {

            for (int i = 0; i < agregados.size(); i++) {
                Query despesa1 = em.createQuery("select d from Despesa d where d.utilizador = '" + agregados.get(i).getAgregado1() + "'", Despesa.class);
                Query despesa2 = em.createQuery("select d from Despesa d where d.utilizador = '" + agregados.get(i).getAgregado2() + "'", Despesa.class);
                despesas.addAll(despesa1.getResultList());
                despesas.addAll(despesa2.getResultList());
            }

            if (despesas.isEmpty()) {
                model.addAttribute("message", "Ainda nenhum elemento do seu agregado familiar adicionou despesas.");
